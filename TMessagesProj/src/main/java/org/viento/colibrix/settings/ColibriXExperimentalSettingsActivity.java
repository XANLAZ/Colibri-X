...
public class ColibriXExperimentalSettingsActivity extends BaseColibriXSettingsActivity {
    ...
    public ColibriXExperimentalSettingsActivity(boolean sensitiveCanChange, boolean sensitiveEnabled) {
    ...
    protected void onItemClick(View view, int position, float x, float y) {
        ...
        if (position == disableFilteringRow) {
            ...
            ColibriXConfig.toggleUnlimitedFavedStickers();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(ColibriXConfig.unlimitedFavedStickers);
            }
            ...
            builder.setPositiveButton(LocaleController.getString("Deactivate", R.string.Deactivate), (dialog, which) -> {
                ...
                for (TLRPC.Dialog TLdialog : dialogs) {
                    ...
                    if (peer.channel_id != 0) {
                        ...
                        if (!chat.broadcast) {
                            getMessageHelper().deleteUserHistoryWithSearch(ColibriXExperimentalSettingsActivity.this, TLdialog.id, 0, null);
                        }
                    }
                    if (peer.user_id != 0) {
            ...
            ColibriXConfig.toggleUnlimitedPinnedDialogs();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(ColibriXConfig.unlimitedPinnedDialogs);
            }
        } else if (position == emojiRow) {
            if (!TextUtils.isEmpty(ColibriXConfig.customEmojiFontPath) && (LocaleController.isRTL && x <= AndroidUtilities.dp(76) || !LocaleController.isRTL && x >= view.getMeasuredWidth() - AndroidUtilities.dp(76))) {
                NotificationsCheckCell checkCell = (NotificationsCheckCell) view;
                ColibriXConfig.toggleCustomEmojiFont();
                checkCell.setChecked(ColibriXConfig.customEmojiFont, 0);
            } else {
            ...
            ColibriXConfig.toggleMapDriftingFix();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(ColibriXConfig.mapDriftingFix);
            }
        } else if (position == showRPCErrorRow) {
            ColibriXConfig.toggleShowRPCError();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(ColibriXConfig.showRPCError);
            }
        }
    }
    ...
    protected boolean onItemLongClick(View view, int position, float x, float y) {
        if (position == emojiRow) {
            try {
                if (ColibriXConfig.customEmojiFont) ColibriXConfig.toggleCustomEmojiFont();
                //noinspection ResultOfMethodCallIgnored
                new File(ColibriXConfig.customEmojiFontPath).delete();
                ColibriXConfig.setCustomEmojiFontPath(null);
            } catch (Exception e) {
                //
            }
            ...
            if (data != null) {
                ...
                if (uri != null) {
                    try {
                        ...
                        if (ColibriXConfig.setCustomEmojiFontPath(dest.toString())) {
                            if (!ColibriXConfig.customEmojiFont) ColibriXConfig.toggleCustomEmojiFont();
                        } else {
    ...
    private class ListAdapter extends BaseListAdapter {
            ...
            switch (holder.getItemViewType()) {
                ...
                case 3: {
                    ...
                    if (position == disableFilteringRow) {
                        ...
                        textCell.setTextAndValueAndCheck(LocaleController.getString("UnlimitedFavoredStickers", R.string.UnlimitedFavoredStickers), LocaleController.getString("UnlimitedFavoredStickersAbout", R.string.UnlimitedFavoredStickersAbout), ColibriXConfig.unlimitedFavedStickers, true, true);
                    } else if (position == unlimitedPinnedDialogsRow) {
                        textCell.setTextAndValueAndCheck(LocaleController.getString("UnlimitedPinnedDialogs", R.string.UnlimitedPinnedDialogs), LocaleController.getString("UnlimitedPinnedDialogsAbout", R.string.UnlimitedPinnedDialogsAbout), ColibriXConfig.unlimitedPinnedDialogs, true, true);
                    } else if (position == mapDriftingFixRow) {
                        textCell.setTextAndCheck(LocaleController.getString("MapDriftingFix", R.string.MapDriftingFix), ColibriXConfig.mapDriftingFix, true);
                    } else if (position == showRPCErrorRow) {
                        textCell.setTextAndValueAndCheck(LocaleController.getString("ShowRPCError", R.string.ShowRPCError), LocaleController.formatString("ShowRPCErrorException", R.string.ShowRPCErrorException, "FILE_REFERENCE_EXPIRED"), ColibriXConfig.showRPCError, true, false);
                    }
                    break;
                }
                case 4: {
                ...
                case 5: {
                    ...
                    if (position == emojiRow) {
                        textCell.setTextAndValueAndCheck(LocaleController.getString("CustomEmojiTypeface", R.string.CustomEmojiTypeface), new File(ColibriXConfig.customEmojiFontPath).getName(), ColibriXConfig.customEmojiFont, true);
                    }
                    break;
                }
            }
        ...
        public int getItemViewType(int position) {
            if (position == experiment2Row || position == deleteAccount2Row) {
                ...
                return TextUtils.isEmpty(ColibriXConfig.customEmojiFontPath) ? 2 : 5;
            }
            return 2;
        }
    }
