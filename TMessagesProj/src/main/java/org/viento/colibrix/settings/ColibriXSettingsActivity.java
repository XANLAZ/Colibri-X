...
import org.viento.colibrix.accessibility.AccessibilitySettingsActivity;
import org.viento.colibrix.helpers.PasscodeHelper;
import org.viento.colibrix.helpers.remote.ConfigHelper;
import org.viento.colibrix.helpers.remote.UpdateHelper;

public class ColibriXSettingsActivity extends BaseColibriXSettingsActivity implements NotificationCenter.NotificationCenterDelegate {
    ...
    protected void onItemClick(View view, int position, float x, float y) {
        if (position == chatRow) {
            presentFragment(new ColibriXChatSettingsActivity());
        } else if (position == generalRow) {
            presentFragment(new ColibriXGeneralSettingsActivity());
        } else if (position == appearanceRow) {
            presentFragment(new ColibriXAppearanceSettings());
        } else if (position == passcodeRow) {
            presentFragment(new ColibriXPasscodeSettingsActivity());
        } else if (position == experimentRow) {
            presentFragment(new ColibriXExperimentalSettingsActivity(sensitiveCanChange, sensitiveEnabled));
            ...
            if (ColibriXConfig.installedFromPlay) {
                presentFragment(new ColibriXDonateActivity());
            } else {
                Browser.openUrl(getParentActivity(), "https://www.buymeacoffee.com/colibrix");
            }
        } else if (position == translationRow) {
            Browser.openUrl(getParentActivity(), "https://colibrix.crowdin.com/colibrix");
        } else if (position == websiteRow) {
            Browser.openUrl(getParentActivity(), "https://appsbox.ru");
            ...
            if (LocaleController.isRTL && x <= AndroidUtilities.dp(84) || !LocaleController.isRTL && x >= view.getMeasuredWidth() - AndroidUtilities.dp(84)) {
                Browser.openUrl(getParentActivity(), "https://githab.com/XANLAZ/Colibri-X");
            }
    ...
    protected String getActionBarTitle() {
        return LocaleController.getString("ColibriXSettings", R.string.ColibriXSettings);
    }
            ...
            switch (holder.getItemViewType()) {
                ...
                case 2: {
                    ...
                    if (position == channelRow) {
                        ...
                        textCell.setTextAndValue(LocaleController.getString("OfficialSite", R.string.OfficialSite), "appsbox.ru", true);
                    } else if (position == sourceCodeRow) {
                ...
                case 8: {
                    ...
                    if (position == chatRow) {
                        ...
                        textCell.setTextAndIcon(LocaleController.getString("PasscodeColibriX", R.string.PasscodeColibriX), R.drawable.msg_permissions, true);
                    } else if (position == experimentRow) {
