...
public abstract class BaseColibriXSettingsActivity extends BaseFragment {
            ...
            if (key != null && holder != null && listAdapter.isEnabled(holder) && rowMapReverse.containsKey(position)) {
                showDialog(new AlertDialog.Builder(context)
                        .setItems(
                                ...
                                (dialogInterface, i) -> {
                                    AndroidUtilities.addToClipboard(String.format(Locale.getDefault(), "https://%s/colibrixsettings/%s?r=%s", getMessagesController().linkPrefix, getKey(), rowMapReverse.get(position)));
                                    BulletinFactory.of(BaseColibriXSettingsActivity.this).createCopyLinkBulletin().show();
                                })
                        .create());
                return true;
            }
    ...
    public void onProvideAssistContent(AssistContent outContent) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            outContent.setWebUri(Uri.parse("https://appsbox.ru"));
        }
    }
}
