...
public class ColibriXLanguagesSelectActivity extends BaseColibriXSettingsActivity {
    ...
    public ColibriXLanguagesSelectActivity(int type, boolean whiteActionBar) {
    ...
    protected void onItemClick(View view, int position, float x, float y) {
        ...
        if (localeInfo != null) {
            if (currentType == TYPE_RESTRICTED) {
                ...
                boolean remove = ColibriXConfig.restrictedLanguages.contains(localeInfo.langCode);
                if (remove) {
                    ColibriXConfig.restrictedLanguages.removeIf(s -> s != null && s.equals(localeInfo.langCode));
                } else {
                    ColibriXConfig.restrictedLanguages.add(localeInfo.langCode);
                }
                ColibriXConfig.saveRestrictedLanguages();
                cell.setChecked(!remove);
            } else {
                ColibriXConfig.setTranslationTarget(localeInfo.langCode);
                finishFragment();
            }
        }
    }
        ...
        public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
            switch (holder.getItemViewType()) {
                ...
                case 9: {
                    ...
                    boolean checked = ColibriXConfig.restrictedLanguages.contains(localeInfo.langCode) || localeInfo.langCode.equals(getCurrentTargetLanguage());
                ...
                case 10: {
                    ...
                    if (localeInfo.langCode.equals("app")) {
                        cell.setTextAndCheck(LocaleController.getString("TranslationTargetApp", R.string.TranslationTargetApp), ColibriXConfig.translationTarget.equals(localeInfo.langCode), !last);
                    } else {
                        cell.setTextAndValueAndCheck(localeInfo.name, localeInfo.nameLocalized, ColibriXConfig.translationTarget.equals(localeInfo.langCode), false, !last);
                    }
                    break;
                }
            }
