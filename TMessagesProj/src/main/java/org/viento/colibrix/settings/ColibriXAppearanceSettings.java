...
public class ColibriXAppearanceSettings extends BaseColibriXSettingsActivity {
    ...
    protected void onItemClick(View view, int position, float x, float y) {
        if (position == hidePhoneRow) {
            ColibriXConfig.toggleHidePhone();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(ColibriXConfig.hidePhone);
            }
            ...
            types.add(ColibriXConfig.TABLET_AUTO);
            arrayList.add(LocaleController.getString("Enable", R.string.Enable));
            types.add(ColibriXConfig.TABLET_ENABLE);
            arrayList.add(LocaleController.getString("Disable", R.string.Disable));
            types.add(ColibriXConfig.TABLET_DISABLE);
            PopupHelper.show(arrayList, LocaleController.getString("TabletMode", R.string.TabletMode), types.indexOf(ColibriXConfig.tabletMode), getParentActivity(), view, i -> {
                ColibriXConfig.setTabletMode(types.get(i));
            ...
            ColibriXConfig.toggleUseSystemEmoji();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(ColibriXConfig.useSystemEmoji);
            }
            ...
            PopupHelper.show(arrayList, LocaleController.getString("EventType", R.string.EventType), ColibriXConfig.eventType, getParentActivity(), view, i -> {
                ColibriXConfig.setEventType(i);
            ...
            PopupHelper.show(arrayList, LocaleController.getString("ActionBarDecoration", R.string.ActionBarDecoration), ColibriXConfig.actionBarDecoration, getParentActivity(), view, i -> {
                ColibriXConfig.setActionBarDecoration(i);
            ...
            ColibriXConfig.toggleNewYear();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(ColibriXConfig.newYear);
            }
            showRestartBulletin();
        } else if (position == avatarAsDrawerBackgroundRow) {
            ColibriXConfig.toggleAvatarAsDrawerBackground();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(ColibriXConfig.avatarAsDrawerBackground);
            }
            getNotificationCenter().postNotificationName(NotificationCenter.mainUserInfoChanged);
            TransitionManager.beginDelayedTransition(profilePreviewCell);
            listAdapter.notifyItemChanged(drawerRow, new Object());
            if (ColibriXConfig.avatarAsDrawerBackground) {
                updateRows();
            ...
            ColibriXConfig.toggleAvatarBackgroundBlur();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(ColibriXConfig.avatarBackgroundBlur);
            }
            getNotificationCenter().postNotificationName(NotificationCenter.mainUserInfoChanged);
            listAdapter.notifyItemChanged(drawerRow, new Object());
        } else if (position == avatarBackgroundDarkenRow) {
            ColibriXConfig.toggleAvatarBackgroundDarken();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(ColibriXConfig.avatarBackgroundDarken);
            }
            getNotificationCenter().postNotificationName(NotificationCenter.mainUserInfoChanged);
            listAdapter.notifyItemChanged(drawerRow, new Object());
        } else if (position == disableNumberRoundingRow) {
            ColibriXConfig.toggleDisableNumberRounding();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(ColibriXConfig.disableNumberRounding);
            }
        } else if (position == appBarShadowRow) {
            ColibriXConfig.toggleDisableAppBarShadow();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(ColibriXConfig.disableAppBarShadow);
            }
            ActionBarLayout.headerShadowDrawable = ColibriXConfig.disableAppBarShadow ? null : parentLayout.getResources().getDrawable(R.drawable.header_shadow).mutate();
            parentLayout.rebuildAllFragmentViews(false, false);
        } else if (position == mediaPreviewRow) {
            ColibriXConfig.toggleMediaPreview();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(ColibriXConfig.mediaPreview);
            }
        } else if (position == formatTimeWithSecondsRow) {
            ColibriXConfig.toggleFormatTimeWithSeconds();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(ColibriXConfig.formatTimeWithSeconds);
            }
            LocaleController.getInstance().recreateFormatters();
            parentLayout.rebuildAllFragmentViews(false, false);
        } else if (position == showTabsOnForwardRow) {
            ColibriXConfig.toggleShowTabsOnForward();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(ColibriXConfig.showTabsOnForward);
            }
        } else if (position == hideAllTabRow) {
            ColibriXConfig.toggleHideAllTab();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(ColibriXConfig.hideAllTab);
            }
            ...
            types.add(ColibriXConfig.TITLE_TYPE_TEXT);
            arrayList.add(LocaleController.getString("TabTitleTypeIcon", R.string.TabTitleTypeIcon));
            types.add(ColibriXConfig.TITLE_TYPE_ICON);
            arrayList.add(LocaleController.getString("TabTitleTypeMix", R.string.TabTitleTypeMix));
            types.add(ColibriXConfig.TITLE_TYPE_MIX);
            PopupHelper.show(arrayList, LocaleController.getString("TabTitleType", R.string.TabTitleType), types.indexOf(ColibriXConfig.tabsTitleType), getParentActivity(), view, i -> {
                ColibriXConfig.setTabsTitleType(types.get(i));
    ...
    protected void updateRows() {
        ...
        newYearRow = addRow("newYear");
        eventTypeRow = addRow("eventType");
        actionBarDecorationRow = addRow("actionBarDecoration");
            ...
            switch (holder.getItemViewType()) {
                case 2: {
                    ...
                    if (position == eventTypeRow) {
                        String value;
                        switch (ColibriXConfig.eventType) {
                            case 1:
                        ...
                        String value;
                        switch (ColibriXConfig.actionBarDecoration) {
                            case 1:
                        ...
                        String value;
                        switch (ColibriXConfig.tabletMode) {
                            case ColibriXConfig.TABLET_AUTO:
                                value = LocaleController.getString("TabletModeAuto", R.string.TabletModeAuto);
                                break;
                            case ColibriXConfig.TABLET_ENABLE:
                                value = LocaleController.getString("Enable", R.string.Enable);
                                break;
                            case ColibriXConfig.TABLET_DISABLE:
                            default:
                ...
                case 3: {
                    ...
                    if (position == hidePhoneRow) {
                        textCell.setTextAndCheck(LocaleController.getString("HidePhone", R.string.HidePhone), ColibriXConfig.hidePhone, false);
                    } else if (position == transparentStatusBarRow) {
                        ...
                        textCell.setTextAndCheck(LocaleController.getString("EmojiUseDefault", R.string.EmojiUseDefault), ColibriXConfig.useSystemEmoji, true);
                    } else if (position == newYearRow) {
                        textCell.setTextAndCheck(LocaleController.getString("ChristmasHat", R.string.ChristmasHat), ColibriXConfig.newYear, true);
                    } else if (position == avatarAsDrawerBackgroundRow) {
                        textCell.setTextAndCheck(LocaleController.getString("AvatarAsBackground", R.string.AvatarAsBackground), ColibriXConfig.avatarAsDrawerBackground, true);
                    } else if (position == disableNumberRoundingRow) {
                        textCell.setTextAndValueAndCheck(LocaleController.getString("DisableNumberRounding", R.string.DisableNumberRounding), "4.8K -> 4777", ColibriXConfig.disableNumberRounding, true, true);
                    } else if (position == appBarShadowRow) {
                        textCell.setTextAndCheck(LocaleController.getString("DisableAppBarShadow", R.string.DisableAppBarShadow), ColibriXConfig.disableAppBarShadow, true);
                    } else if (position == mediaPreviewRow) {
                        textCell.setTextAndCheck(LocaleController.getString("MediaPreview", R.string.MediaPreview), ColibriXConfig.mediaPreview, true);
                    } else if (position == formatTimeWithSecondsRow) {
                        textCell.setTextAndCheck(LocaleController.getString("FormatWithSeconds", R.string.FormatWithSeconds), ColibriXConfig.formatTimeWithSeconds, true);
                    } else if (position == avatarBackgroundBlurRow) {
                        textCell.setTextAndCheck(LocaleController.getString("BlurAvatarBackground", R.string.BlurAvatarBackground), ColibriXConfig.avatarBackgroundBlur, true);
                    } else if (position == avatarBackgroundDarkenRow) {
                        textCell.setTextAndCheck(LocaleController.getString("DarkenAvatarBackground", R.string.DarkenAvatarBackground), ColibriXConfig.avatarBackgroundDarken, true);
                    } else if (position == showTabsOnForwardRow) {
                        textCell.setTextAndCheck(LocaleController.getString("ShowTabsOnForward", R.string.ShowTabsOnForward), ColibriXConfig.showTabsOnForward, true);
                    } else if (position == hideAllTabRow) {
                        textCell.setTextAndValueAndCheck(LocaleController.getString("HideAllTab", R.string.HideAllTab), LocaleController.getString("HideAllTabAbout", R.string.HideAllTabAbout), ColibriXConfig.hideAllTab, true, true);
                    }
                    break;
                }
                case 4: {
        ...
        public int getItemViewType(int position) {
            if (position == appearance2Row || position == drawer2Row) {
                return 1;
                    ...
                    position == tabsTitleTypeRow || position == tabletModeRow) {
                return 2;
