...
public class DrawerProfileCell extends FrameLayout {
    ...
    public DrawerProfileCell(Context context, DrawerLayoutContainer drawerLayoutContainer) {
        ...
        imageReceiver.setDelegate((imageReceiver, set, thumb, memCache) -> {
            if (ColibriXConfig.avatarBackgroundDarken || ColibriXConfig.avatarBackgroundBlur) {
                if (thumb) {
                    return;
                }
                ...
                if (bmp != null) {
                    new Thread(() -> {
                        int width = ColibriXConfig.avatarBackgroundBlur ? 150 : bmp.bitmap.getWidth();
                        int height = ColibriXConfig.avatarBackgroundBlur ? 150 : bmp.bitmap.getHeight();
                        Bitmap bitmap = Bitmap.createBitmap(width, height, Bitmap.Config.ARGB_8888);
                        Canvas canvas = new Canvas(bitmap);
                        canvas.drawBitmap(bmp.bitmap, null, new Rect(0, 0, width, height), new Paint(Paint.FILTER_BITMAP_FLAG));
                        if (ColibriXConfig.avatarBackgroundBlur) {
                            try {
                        ...
                        if (ColibriXConfig.avatarBackgroundDarken) {
        ...
        if (Theme.getEventType() == 0 || ColibriXConfig.actionBarDecoration  == 1) {
            snowflakesEffect = new SnowflakesEffect(0);
            snowflakesEffect.setColorKey(Theme.key_chats_menuName);
        } else if (ColibriXConfig.actionBarDecoration == 2) {
