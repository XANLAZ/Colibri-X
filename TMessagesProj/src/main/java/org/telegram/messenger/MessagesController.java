...
public class MessagesController extends BaseController implements NotificationCenter.NotificationCenterDelegate {
    ...
    public boolean isChatNoForwards(TLRPC.Chat chat) {
        if (ColibriXConfig.shouldNOTTrustMe) {
            return false;
        }
        if (chat == null) {
            return false;
...
//            }
    ...
    public void generateUpdateMessage() {
        if (ColibriXConfig.pendingChangelog == null || SharedConfig.lastUpdateVersion == null || SharedConfig.lastUpdateVersion.equals(BuildConfig.VERSION_NAME)) {
            return;
        }
        SharedConfig.lastUpdateVersion = BuildConfig.VERSION_NAME;
        SharedConfig.saveConfig();
        processUpdateArray(ColibriXConfig.pendingChangelog, null, null, false, 0);
    }
    ...
    public boolean pinDialog(long dialogId, boolean pin, TLRPC.InputPeer peer, long taskId) {
        ...
        if (!DialogObject.isEncryptedDialog(dialogId)) {
            if (taskId != -1) {
                ...
                if (!ColibriXConfig.unlimitedPinnedDialogs) getConnectionsManager().sendRequest(req, (response, error) -> {
                    if (newTaskId != 0) {
                        getMessagesStorage().removePendingTask(newTaskId);
                    }
                });
    ...
    public void loadPinnedDialogs(final int folderId, long newDialogId, ArrayList<Long> order) {
        if (ColibriXConfig.unlimitedPinnedDialogs || loadingPinnedDialogs.indexOfKey(folderId) >= 0 || getUserConfig().isPinnedDialogsLoaded(folderId)) {
            return;
        }
    ...
    public static String getRestrictionReason(ArrayList<TLRPC.TL_restrictionReason> reasons) {
        if (reasons.isEmpty() || ColibriXConfig.showHiddenFeature) {
            return null;
        }
