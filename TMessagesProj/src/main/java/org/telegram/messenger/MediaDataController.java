...
public class MediaDataController extends BaseController {
    ...
    public ArrayList<TLRPC.Document> getRecentStickers(int type) {
        ...
        return new ArrayList<>(arrayList.subList(0, Math.min(arrayList.size(), ColibriXConfig.maxRecentStickers)));
    }
    ...
    public void addRecentSticker(int type, Object parentObject, TLRPC.Document document, int date, boolean remove) {
        ...
        if (type == TYPE_FAVE) {
            ...
            maxCount = ColibriXConfig.unlimitedFavedStickers ? Integer.MAX_VALUE : getMessagesController().maxFaveStickersCount;
        } else {
    ...
    public void loadRecents(int type, boolean gif, boolean cache, boolean force) {
        if (ColibriXConfig.unlimitedFavedStickers && type == TYPE_FAVE && !cache) {
            return;
        }
        if (gif) {
